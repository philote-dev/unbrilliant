...
export function Profile() {
  ...
  if (isDesktop) return <ProfileDesktop />
...
function SettingsEntry({ className }: { className?: string }) {
  const { navigate } = useNavigation()
  ...
  return (
    <button
      ...
      className={
        "flex w-full items-center gap-3 rounded-3xl border border-border bg-card px-5 py-4 shadow-card transition-colors hover:bg-muted" +
        (className ? ` ${className}` : "")
      }
    >

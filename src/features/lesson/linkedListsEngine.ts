...
function makeWalk(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: "Walk from the head, counting links — no jumping ahead. Tap where you land.",
...
function makeInsert(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: `Save the rest first: aim ${NEW_NODE} at ${at}, then aim ${prev} at ${NEW_NODE}.`,
...
function makeDelete(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: `Point ${prev} straight past ${cur}, on to ${after}.`,
...
function makePredict(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: "Picture the pointer that just got overwritten.",
...
function makePlaylist(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: `Same two writes: aim the new track at ${at}, then aim ${prev} at it.`,
...
function makeContrastInsert(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      cost: { word: "free", count: 2, unit: "pointers rewired" },
      hint: "Picture making room in a packed row vs re-aiming two arrows.",
...
function makeContrastReach(seed: number): { question: LLQuestion; next: number } {
  ...
  return {
    question: {
      ...
      hint: "One structure indexes directly; the other has no address math.",
